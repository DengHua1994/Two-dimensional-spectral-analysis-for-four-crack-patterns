function [IrData,IomData]=spectral2D(Filename,Options,name);
...
AngleBinSize=10;            % The size of an angle bin in the angular spectrum
Resolution=0.2;             % 1 pixel is xxx meters;
...
%     subplot(2,2,1);
    ...
    set(gca,'fontsize',fs,'Layer','top','linewidth',2,'TickDir','out','TickLength',[0.02 0.01]);          
...
%     subplot(2,2,2);
    ...
    plot(s/2+1,s/2+1,'*');
    set(gcf, 'Position', [0 0 600 600],'color','w','Visible','on');% Set the image size, the same below
    set(gca,'Fontname','times new Roman')
    set(gca,'fontsize',fs,'Layer','top','linewidth',2,'TickDir','in','TickLength',[0.02 0.01]);

%     subplot(2,2,3); 
    nexttile;
    plot(1:rmax,Ir(1:end),'-s','markersize',10,'linewidth',2);hold on;
    plot(1:rmax,Chi_up_Rad(1:end),'b.','markersize',10);
    plot(1:rmax,Chi_dw_Rad(1:end),'b.','markersize',10);
...
saveas(gcf,strcat(name,'.fig'));%save picture
